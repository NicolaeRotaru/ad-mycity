...
data: 2026-07-19 13:01
fonte: Supabase REST live (`verifica-sensori.mjs` 13:01 `supabase_rest=ok`) · `abandoned_carts` + `profiles` + `coupons` + `orders`
...
# Recupero carrelli — pacchetto pronto (19/7 13:01 · RIPROVA Nicola 10:52)

> **Verifica live 2026-07-19 13:01.** REST marketplace leggibile (`supabase_rest=ok`).
> `abandoned_carts`: **4 record** (invariati vs 14/7). `profiles`: **23** totali · **4 buyer** · solo **1** con carrello fermo.
...
|---|---|---|
| Record `abandoned_carts` | **4** | REST 19/7 13:01 |
| Carrelli recuperabili reali | **1** | unico `role=buyer` non-seed (samir) |
| Altri 3 record | admin / seller-auto-test / seed Casa Linda | **SKIP** |
| Profili totali | **23** | `profiles` REST 19/7 |
| Buyer registrati | **4** | `profiles?role=buyer` |
| Ordini nel DB | **1** (unico, `delivery_status=CANCELED` 3/7) | `orders` REST 19/7 |
| Consenso marketing buyer samir | `email_marketing = false` | `profiles` |
| Coupon attivi | `BENVENUTO10` (10%, primo ordine) · `SPED5` (€5 sopra €25) | `coupons` REST 19/7 |

## Classificazione (19/7 13:01)

| # | user_id | Nome | Ruolo | Totale | Fermo da | Inviabile? |
|---|---------|------|-------|--------|----------|------------|
| 1 | `57494b3e-fd67-4379-8b9c-90e40e39ff06` | samir? | buyer | **€10,00** | **~781h (16/6, ~33 giorni)** | ✅ **SÌ** — unico cliente reale |
| 2 | `33333333-3333-3333-3333-aaaaaaaa0001` | Assistenza MyCity | admin | €7,95 | ~402h (2/7) | ⏭️ SKIP — admin test |
| 3 | `11111111-1111-1111-1111-cccccccc0001` | Casa Linda | seller | €17,80 | ~764h (17/6) | ⏭️ SKIP — seed demo |
| 4 | `c0b240c0-2a86-4218-9d0f-5154f08ff929` | Pane Quotidiano | seller | €13,90 | ~783h (16/6) | ⏭️ SKIP — auto-test negoziante |
...
3. Mani Resend: accese lato API (`verifica-sensori` 13:01) ma invio reale resta 🔴 firma Nicola
...
|---|---|
| [[AZIONI-PRONTE]] **A3** | Touch #1 🟡 + Touch #2 🔴 (agg. 19/7 13:01) |
