...
4. **Ultime rotaie di carta:** `cervello/README.md` (era impostato su Cursor in 5 punti — ora corretto) e `giro.md` (ramo cloud `claude/…`).
5. **Fatto-chiave nel registro (AR-102):** `motore-principale` = Claude, con 3 cacce attive sui pattern ereditati — se una copia vecchia rispunta in un file vivo, il guardiano del giro FALLISCE. Verificato: 670 file scansionati, exit 0.
