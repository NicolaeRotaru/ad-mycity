---
data: 2026-07-20 00:03
tipo: diagnosi
reparto: finanza
origine: sentinella cassa_sconosciuta (171 giri, worker 20/7 00:02)
---

# Diagnosi cassa / runway — aggiornata 20 luglio 2026

## In una riga per Nicola

**Stripe funziona e legge 0€; manca solo `BURN_MENSILE_EUR` nel `.env` del VPS — senza quel numero il runway resta «sconosciuto» da 171 giri. Non è un bug Stripe.**

---

## Cosa abbiamo verificato adesso (00:03, VPS)

| Pezzo | Stato | Prova |
|---|---|---|
| `STRIPE_SECRET_KEY` | ✅ collegata | `verifica-sensori.mjs` exit 0 → `stripe_api: balance API ok` |
| Cassa Stripe (available + pending) | **0 €** | `sensore-cassa.mjs` 00:03 |
| `BURN_MENSILE_EUR` | ❌ assente | `grep -c` su `cervello/vps/.env` → 0 occorrenze |
| Runway | **sconosciuto** | 171 giri consecutivi (contatore AR-039) |
...
| **20/7 00:03** | **171** | invariato — unica azione = env burn |
