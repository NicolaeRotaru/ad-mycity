---
data: 2026-07-20 02:07
tipo: diagnosi
reparto: finanza
origine: sentinella cassa_sconosciuta (175 giri, worker 20/7 02:07)
---
...
**Stripe funziona e legge 0€; manca solo `BURN_MENSILE_EUR` nel `.env` del VPS — senza quel numero il runway resta «sconosciuto» da 175 giri. Non è un bug Stripe.**

---

## Cosa abbiamo verificato adesso (02:07, VPS)

| Pezzo | Stato | Prova |
|---|---|---|
| `STRIPE_SECRET_KEY` | ✅ collegata | `verifica-sensori.mjs` exit 0 → `stripe_api: balance API ok` |
| Cassa Stripe (available + pending) | **0 €** | `sensore-cassa.mjs` 02:07 |
| `BURN_MENSILE_EUR` | ❌ assente | `grep -c` su `cervello/vps/.env` → 0 occorrenze |
| Runway | **sconosciuto** | 175 giri consecutivi (contatore AR-039) |
...
**Card già in coda:** 🟡 `#burn-mensile-env` (approvata 16–17/7, **non ancora eseguita** sul VPS). Non ri-accodata.
...
| 20/7 01:02 | 173 | invariato |
| **20/7 02:07** | **175** | invariato — unica azione = env burn |
