---
data: 2026-07-16 01:07
tipo: diagnosi
reparto: finanza
origine: sentinella cassa_sconosciuta (109 giri)
---
...
**Stripe funziona e legge 0€ in cassa; manca solo che tu dica quanto spendiamo al mese (`BURN_MENSILE_EUR`) — senza quel numero il runway resta «sconosciuto» da 109 giri.**

---

## Cosa abbiamo verificato (01:07, VPS)
...
| `BURN_MENSILE_EUR` | ❌ assente | né in `vps/.env` né in `.env` root |
| Runway | **sconosciuto** | 109 giri consecutivi senza calcolo |
| Ordini pagati | 0 | nessun incasso marketplace in cassa |

**Non è un bug Stripe.** Il sensore legge la cassa. Blocco reale = **solo burn mensile mancante**.
...
Card **🟡 #burn-mensile-runway** — «Imposta quanto spendiamo al mese per calcolare il runway» — in Da approvare dal 14/7. **Non serve ri-accodare.** Basta firmare con il numero reale (anche stima va bene per partire).
...
*Fonti: `cassa-runway.json` 16/7 01:07 · `sensore-cassa.mjs` + `verifica-sensori.mjs` exit 0 · `sensori-cecita.json` · regola AR-039/AR-016*
