...
data: 2026-07-16 11:11
fonte: Supabase REST live (`orders` + `store_reviews` + `reviews`) · `verifica-sensori.mjs` 11:10 (`supabase_rest=ok`)
...
# Recensioni post-consegna — pacchetto pronto (16/7 11:11 · playbook worker)

> **Verifica live 2026-07-16 11:11.** REST marketplace leggibile (`supabase_rest=ok`).
...
|---|---|---|
| Ordini totali in DB | **1** | REST 16/7 11:11 |
...
| **A27** | Indice ri-verifica playbook | aggiornato 16/7 11:11 |
...
2. **Alla prima consegna PQ (Venerdì Piacentini 17/7):** invio manuale A13 → se 👍 A14 (oppure firma **#recensioni-trigger** per automazione email).
