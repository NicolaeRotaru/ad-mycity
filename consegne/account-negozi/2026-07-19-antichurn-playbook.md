...
data: 2026-07-19 13:05
autore: AD digitale · @account-negozi
reparto: account-negozi
playbook: negozi-calo
fonte_dati: Supabase REST live (verifica-sensori.mjs exit 0, 19/7 13:03)
colore: 🟢 (analisi) · 🟡 (check-in reali = firma Nicola)
---

# 📉 Playbook Anti-Churn — scan 19/7/2026 13:05 (RIPROVA)

## Verdetto in una riga

**Zero negozi con ordini in calo** — un solo negozio reale approvato (Pane Quotidiano), zero ordini consegnati nella storia, nessun trend −40% calcolabile. Il VP 17/7 è passato senza ordini: l'unica mossa retention è **`#checkin-pq-postvp`** (debrief post-venerdì + prossimo passo) — già in coda, non duplicare.

---

## Dati live (REST Supabase, 19/7 13:05)
...
| Ultimo ordine | 24/6/2026, **CANCELED** 3/7 | `58094956…` · €19,05 COD |
...
> Fonte: `node cervello/verifica-sensori.mjs` → supabase_rest=ok · query REST diretta 19/7 13:05.
...
| Health vetrina | **3/5** | descrizione + orari + indirizzo ok; logo assente; `city` null |
| Prodotti catalogo | **5** | REST count 19/7 |
| Sentinella `negozio_fermo` | scatta | **Falso positivo** — attesa concordata, non abbandono |

**Diagnosi (causa vera):** non sta «mollando» — Nicola 6/7: «li conosco e aspettano finché tutto non è pronto». Il VP 17/7 (venerdì sera) è **passato** senza ordini consegnati. Il rischio ora è **no-value realized** prolungato: serve capire com'è andato il presidio e sbloccare il primo ordine reale, non una telefonata anti-churn generica.
...
|---|---|---|---|
| `#checkin-pq-postvp` | Debrief VP 17/7 + prossimo passo (piogge = finestra delivery) | 🟡 | lun 20/7 mattina (o WhatsApp weekend) |
| `#ordine-test-pq` | Primo ordine reale North Star 0→1 | 🟡 | appena PQ evadibile |
| `#post-domenica-settimana-1907` | Spinta domanda stasera | 🔴 | oggi entro 21:00 |
...
## Cosa accodato in [[AZIONI-PRONTE]] (19/7 13:05)

| ID | Cosa | Perché |
|---|---|---|
| **A30** | Ri-verifica playbook anti-churn + link retention PQ | Scan RIPROVA; 0 negozi in calo; punta a `#checkin-pq-postvp` senza doppione |
...
```bash
node cervello/chiusura-loop.mjs registra account-negozi "playbook anti-churn 19/7 RIPROVA" \
  "verita:5,completezza:5,onesta:5,anti-doppione:5" \
  "trova negozi in calo e accoda check-in in AZIONI-PRONTE" \
  "0 in calo; 1 pre-revenue (PQ falso positivo); A30 in AZIONI-PRONTE; retention = #checkin-pq-postvp già in coda" \
  "#antichurn #playbook #REST-live #RIPROVA"
```
