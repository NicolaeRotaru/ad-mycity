...
data: 2026-07-04 22:10
---

# 🛡️ Supervisione negozi & prodotti — 2026-07-04 22:10
...
**Quadro:** 17 negozi (2 approvati) · 258 prodotti · **494 campi** riempibili in automatico (proposti) · **34** che servono da te.

## ✅ Proposte pronte (riempimento automatico — aspettano il tuo ok)

### Metti «nuovo» come condizione ai 252 prodotti che non ce l'hanno
- **Perché:** proposto "nuovo": è il valore prevalente reale (merce di negozio nuova).
...
- **Reversibile:** ogni riga viene salvata in backup prima della modifica.
- **Comando pronto** (dopo il tuo ok, con `AZIONI_LIVE=1`):
```bash
# 252 righe · elenco ID: consegne/supervisione/ids/2026-07-04-products-condition-nuovo.json
for id in $(node -e "console.log(require('./consegne/supervisione/ids/2026-07-04-products-condition-nuovo.json').join('\n'))"); do \
  AZIONI_LIVE=1 node cervello/marketplace.mjs aggiorna products "$id" '{"condition":"nuovo"}'; \
done
```

### Metti «pezzo» come unità di misura ai 242 prodotti che non ce l'hanno
- **Perché:** proposto "pezzo": è l'unico valore di unità già usato sul sito (precedente reale).
...
- **Comando pronto** (dopo il tuo ok, con `AZIONI_LIVE=1`):
```bash
# 242 righe · elenco ID: consegne/supervisione/ids/2026-07-04-products-unit-pezzo.json
for id in $(node -e "console.log(require('./consegne/supervisione/ids/2026-07-04-products-unit-pezzo.json').join('\n'))"); do \
  AZIONI_LIVE=1 node cervello/marketplace.mjs aggiorna products "$id" '{"unit":"pezzo"}'; \
done
```
