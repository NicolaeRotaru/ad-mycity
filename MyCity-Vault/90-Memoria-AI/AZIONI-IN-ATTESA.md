...
| 14 | 2026-07-01 07:41 | devops | **Token GitHub write su `NicolaeRotaru/mycity`** — estendere `GIT_PUSH_TOKEN` (fine-grained) con *Contents: Read and write* su repo **mycity** **oppure** aggiungere `MARKETPLACE_GIT_TOKEN` in `cervello/vps/.env` | 🟡 | `cervello/vps/.env.example` · oggi PAT scrive solo su `ad-mycity` | GitHub PAT + env VPS | in attesa | L'AD può pushare branch e aprire PR sul marketplace senza passare da te ogni volta. | Dopo **`ok push Sprint 1`** → PR #Sprint1 su mycity in automatico. |
| 15 | 2026-07-01 10:22 | devops→AD | **Token GitHub merge (`GITHUB_MERGE_TOKEN`)** — PAT fine-grained su **`ad-mycity` + `mycity`**: *Contents* + *Pull requests: Read and write* · incolla in `cervello/vps/.env` (può coincidere con `GIT_PUSH_TOKEN` / `MARKETPLACE_GIT_TOKEN` se stesso PAT) · guida: chat AD 1/7 10:45 + `cervello/vps/.env.example` | 🟡 | Nicola: «impara flusso merge-on-approval» · istruzioni PAT consegnate 10:45 | GitHub PAT + env VPS + `systemctl restart mycity-worker` | in attesa | Dopo il tuo ok esplicito l'AD mergia la PR su GitHub al posto tuo — niente browser. Vercel redeploya il Pannello; opzionale sync VPS. | Card «Merge PR #N?» nel Pannello + un click «Ok merge» chiude il loop deploy 🟡. Deploy prod marketplace resta 🔴 separato. |
| 16 | 2026-07-01 11:05 | operations→supporto | **Eseguire Scelta A ordine zombie €19,05 — Pane Quotidiano** · approvato **`ok 16`** Pannello **2/7 08:38** · pacchetto esecuzione pranzo | 🔴 | `consegne/operations/2026-07-02-esecuzione-ok16-pranzo.md` | WhatsApp + dashboard + COD (manuale) | ⏳ **IN ESECUZIONE** ok 16 · passi #20–#22 | Nicola ha dato il via: partono i contatti reali e la consegna COD oggi pranzo — prima transazione MyCity. | Ordine consegnato + COD incassato → A13/A14 recensione + payout-test 03/7. |
| 20 | 2026-07-02 08:38 | operations→supporto | **#16.1 WhatsApp buyer 348 642 1766** — messaggio pranzo 12:00–13:30 + chiedi indirizzo reale (placeholder in DB) · link wa.me in pacchetto ok16 | 🔴 | `consegne/operations/2026-07-02-esecuzione-ok16-pranzo.md` § PASSO 1 | WhatsApp (Nicola) | in attesa | Il buyer riceve il messaggio reale con slot pranzo e conferma dove consegnare. | Buyer risponde sì → passi #21–#22. |
...
| 23 | 2026-07-02 10:00 | builder→AD | **Fix AR-001 — Sensori MCP:** retry + fallback REST (`cervello/vps/.env`) + contatore «giri ciechi» in `giro.md` + aggiornamento `sensori-cecita.json` | 🟡 | `cervello/calibrazione.md` · proposta in `auto-miglioramento.json` · AR-001 cantiere | codice cervello | in attesa | La macchina smette di lavorare al buio: o legge i dati live o dichiara il gap con conteggio esplicito. | Calibrazione previsto-vs-reale parte su numeri veri ogni giro. |
| 24 | 2026-07-02 10:00 | tech→AD | **Fix AR-002 — MARKETPLACE_REPO:** parametrizzare path in `radiografia.js` e `audit-design.js` (env + fallback) | 🟡 | `.claude/workflows/radiografia.js` · AR-002 cantiere | codice cervello | in attesa | La radiografia del sito gira sul VPS Linux, non solo sul PC Windows. | Audit marketplace schedulabile; salute sito entra nel «sé» automaticamente. |
| 25 | 2026-07-02 10:00 | builder→AD | **Fix AR-003 — Sensore cecità dati:** nuovo `cervello/sensori-cecita.mjs` + sentinella + blocco in sonda auto-radiografia | 🟡 | proposta Fase 2 Piano Massimo Potenziale · AR-003 cantiere | codice cervello | in attesa | Quando un sensore è cieco da N giri, la Cabina lo mostra e scatta allerta prima del silenzio. | Nicola vede subito se il problema è tecnico o decisionale. |
