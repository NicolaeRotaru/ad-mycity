...
| 13 | 2026-07-02 23:40 | AD/Tech (builder-automazioni) | **Attivare la sentinella dei DATI sul VPS** (veglia in tempo reale ogni 2 min, 0 token): `sudo bash /opt/mycity/ad-mycity/cervello/vps/install-ritmo-timers.sh` (dopo il `git pull` del branch mergiato). Verifica: `systemctl list-timers \| grep sentinella-dati`. Consiglio: prima un dry-run `node cervello/sentinella-dati.mjs` per vedere cosa accoderebbe | 🔴 | codice in branch `claude/worker-sentinel-analysis-ncldvf` · analisi in `consegne/automazioni/2026-07-02-sentinella-dati-veglia-realtime.md` | VPS (systemctl, lato Nicola/root) | in attesa | La macchina inizia a guardare i dati reali ogni 2 minuti a costo zero e sveglia il modello AI **solo** quando scatta una soglia (ordine senza payout, calo ordini, sensore cieco…). Nessun token sprecato a vuoto. | La Cabina reagisce in minuti invece che al prossimo giro (ogni 2h); i problemi 🔴 arrivano subito su Telegram e come proposta da firmare. |
