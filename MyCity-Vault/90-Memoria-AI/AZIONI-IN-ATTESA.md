...
**Cosa cambia:** il comando «radiografia» oggi non parte. Il motore dei workflow è cambiato e adesso pretende che il file cominci con la sua scheda di presentazione, mentre il nostro comincia con tre righe tecniche prima: il motore lo rifiuta senza nemmeno provarci. Oggi me ne sono accorto solo perché l'ho lanciato io e l'ho aggirato con una copia a mano — ma se lo lanciavi tu, ti tornava un errore e basta. Lo stesso difetto può valere per gli altri quattro comandi che girano allo stesso modo (auto-radiografia, audit del design, audit del pannello, giro operativo): vanno controllati tutti.

**Se va bene:** sposto la scheda di presentazione in cima nei file che ne hanno bisogno, calcolo il percorso del codice del sito senza le righe tecniche di prima, e aggiungo un controllo che a ogni giro prova ad avviare i cinque comandi e diventa rosso se uno non parte — così un comando non può più morire in silenzio fino al giorno in cui ti serve.

**Nota tecnica:** `.claude/workflows/radiografia.js` — `export const meta` deve essere la prima istruzione, oggi è preceduto da tre `import` e la risoluzione di `MARKETPLACE_REPO` gira prima. Stessa forma in `auto-radiografia.js`, `audit-design.js`, `audit-pannello.js`, `giro-operativo.js`: da verificare uno per uno. Aggiungere il controllo al cancello del giro.
