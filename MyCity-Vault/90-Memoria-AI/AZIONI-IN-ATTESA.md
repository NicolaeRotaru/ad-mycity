...
| 197 | 2026-07-19 18:56 | @tech | Merge PR fix build Render (`fix/render-build-orders-page` → main) | 🔴 | branch `fix/render-build-orders-page` repo mycity · body: `consegne/tech/pr-mycity-render-build-body.md` | github | in attesa | Build Render crashato: errore TypeScript pagina ordini (non da #214). Apri PR da GitHub se non compare in Da approvare (rate limit API). | Deploy Render verde (~2–3 min) → poi #215 → ricarica sito da telefono. |
