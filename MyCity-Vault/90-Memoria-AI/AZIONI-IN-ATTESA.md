...
| 17 | 2026-07-01 20:02 | devops | **Attiva sync VPS automatico** — 1 comando root sul VPS (Nicola ha approvato «ok configura sync VPS»; codice già in repo): `sudo bash /opt/mycity/ad-mycity/cervello/vps/install-sync-vps.sh` | 🟡 | `cervello/vps/install-sync-vps.sh` · `mycity-sync-vps.sudoers` | SSH VPS (1× root) | in attesa | Dopo ogni merge su `main` l'AD allinea il worker da solo — non devi più lanciare `aggiorna-cervello.sh` a mano. | Post-merge: dici «ok merge …» → merge GitHub → Vercel redeploy → VPS si aggiorna in automatico (~30s). |
