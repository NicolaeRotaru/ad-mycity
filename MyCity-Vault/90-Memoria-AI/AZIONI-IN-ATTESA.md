...
**Aggiornamento 21/8 03:46 — ancora rotto. È la causa del nuovo allarme "battito fermo".** Ho riletto ora `.claude/settings.local.json`. Le 5 righe sono ANCORA `Write(...)`. Sono identiche a quelle di 5 giorni fa. Il danno nel frattempo si è allargato. Il file che registra l'uscita di ogni cadenza (`esito-cadenze.json`) non riceve una riga nuova dal 18/8 alle 08:56: sono **3 giorni fermo**. Riguarda TUTTE le 6 cadenze — mattino, giro, monitora, mezzogiorno, sera, settimana. Il 15/8 ne erano ferme solo 2 su 6: sta peggiorando. La sentinella lo segnala da sola stanotte come "BATTITO FERMO" (regola AR-592, `cervello/sentinella-dati.mjs`). È il freno automatico che chiedevo: si è acceso da solo, senza bisogno di una mia frase. Il worker che risponde alla chat resta vivo — i commit di stasera lo dimostrano. È fermo solo il lato a orario: i timer che lanciano da soli `giro.sh` e `ritmo-*.sh`. Non riescono più a scrivere in memoria. È lo stesso sintomo di 5 giorni fa, con la stessa causa mai corretta.
...
**Aggiornamento 21/8 05:5x — trovato un secondo buco nello stesso file, diverso dal primo.** Avevo in coda un lavoro della sentinella "salute bassa": 125 difetti aperti nel cantiere, vai alla radice. Ho provato a far girare gli strumenti che servono per CHIUDERE un difetto: `cantiere-prove.mjs`, `cancello-lotto.mjs`, `prove-oneste.mjs`. Ognuno mi ha risposto "richiede approvazione". Ho controllato `.claude/settings.local.json`. Nella lista dei comandi permessi ci sono solo due script node autorizzati per intero: `pulisci-coda.mjs` e `git-pr.mjs`. Tutti gli altri script lanciati come programma a sé (`node cervello/<nome>.mjs`) restano fuori.

Non è tutto bloccato. `node --test cervello/test/<nome>.test.mjs` — il modo in cui girano le prove dei singoli difetti — funziona, l'ho provato con successo. Il buco riguarda solo gli script che orchestrano un lotto intero: il cancello di uscita, il conteggio delle malattie.

Non è lo stesso guasto delle 5 righe Write/Edit sopra. Quello riguarda la scrittura dei file. Questo riguarda l'esecuzione di un tipo di comando. La scrittura infatti funziona: questa nota l'ho scritta senza problemi. Ma non sono riuscita a far girare il cancello di un lotto.

Risultato: solo analisi, nessun fix. Con `jq`/`grep` (sola lettura, non serve permesso) e con `node --test` ho contato 106 difetti aperti nel cantiere, 26 a impatto-crescita "alto", 9 "bloccanti". Il dettaglio è nella nota separata per il reparto tech/cantiere, non qui.
...
**Aggiornamento 21/8 06:2x — il distacco da GitHub sta crescendo, non solo restando fermo.** Ho rifatto `git fetch` e `git status`. Ora sono **81 commit solo qui, 25 solo su GitHub**. Sette minuti fa erano 75 e 25. In quei sette minuti il worker ha prodotto 5 commit di allarme in fila: LOOP APERTO, CHECKUP FERMO, BATTITO FERMO, SENSORI FERMI. Sempre sullo stesso problema mai risolto. Ognuno di questi commit si somma al conteggio "solo qui". Nessuno riesce a raggiungere GitHub. È un circolo vizioso, e ora l'ho misurato: il permesso resta sbagliato, il numero di commit da riconciliare cresce, e sistemarli dopo diventa più rischioso e più lento. Non ho ritentato il rebase. Chi l'ha già provato oggi si è fermato al primo conflitto e ha annullato in sicurezza. Ripeterlo ora, senza tempo dedicato, rischierebbe di perdere lavoro da una delle due parti. Non ho aperto una card nuova: resta questa.

**Cosa fare (proposta, tua firma):** in più, oltre alle righe di permesso già elencate sopra, valuta di mettere in pausa i timer automatici (`mycity-giro.timer`, `mycity-ritmo-*.timer`). Fermano l'accumulo mentre qualcuno risolve con calma la divergenza. Un'idea già proposta in una nota precedente: tenere solo l'ultima fotografia di ogni file di auto-coscienza. Non serve far quadrare tutti i commit intermedi: sono fotografie ripetute nel tempo, non storia che serve davvero.
