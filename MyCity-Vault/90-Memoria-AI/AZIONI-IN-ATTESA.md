...
> 🧹 **Housekeeping 2026-07-23 16:20** — Automatico: **47 aperte · 98 chiuse in archivio**.
...
| 221 | 2026-07-23 11:37 | @devops-sre | Anche il token GitHub del Pannello online (Vercel) è rotto — briefing e audit del sito non si aggiornano più | 🔴 | Nicola ha mandato screenshot del Pannello: in "Diagnosi macchina" **«Vault GitHub (Pannello)» ROSSO** (`accesso KO: GitHub 401`, legge env `OBSIDIAN_TOKEN` con fallback `GITHUB_TOKEN` — `pannello/src/lib/obsidian.ts:7`) e **«Codice marketplace (GitHub)» ROSSO** (`GitHub 401: Bad credentials`, legge `GITHUB_TOKEN` — `pannello/src/lib/github.ts:9`). È un token DIVERSO da quello del VPS (#219, quello serve a scrivere/pushare da qui): questo è il token che **Vercel** usa per leggere GitHub da fuori. Verificato ora: il repo si legge ancora in lettura semplice da questo VPS (`git fetch` ok) — il problema è solo sui due token configurati, non sul repo. **Aggiornamento 23/7 16:04:** Nicola ha rigenerato il token e fatto redeploy — screenshot delle 16:04 mostra ancora ROSSO, stesso errore "Bad credentials". Ipotesi AD (non ancora verificata): su Vercel esiste probabilmente ANCHE la variabile `OBSIDIAN_TOKEN` con il valore vecchio, che ha priorità su `GITHUB_TOKEN` in `obsidian.ts:7` — se non viene aggiornata/rimossa, il nuovo `GITHUB_TOKEN` viene ignorato. Chiesto a Nicola screenshot della lista variabili (solo nomi, mai valori) per confermare. | manuale (Vercel, accesso diretto) | in attesa (2° tentativo fallito, causa non confermata) | Finché è rosso: il Pannello online smette di mostrare i nuovi briefing/STATO in tempo reale (torna alla copia in cache) e gli audit del codice del sito (`mycity`) non si possono più fare. | Su GitHub genera un nuovo PAT (fine-grained, sola lettura, repo `ad-mycity`+`mycity`, scope Contents:Read) → su Vercel → Settings → Environment Variables aggiorna `GITHUB_TOKEN` (e `OBSIDIAN_TOKEN` se lo usi separato) → Redeploy manuale (il push da solo non basta, `deploymentEnabled.main=false`) → ricontrolla `/api/diagnosi` che i due sensori tornino verdi. |
...
## 🛡️ Supervisione negozi & prodotti — proposte di riempimento (aggiornato 2026-07-23 16:20)
