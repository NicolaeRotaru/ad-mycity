...
| 124 | 2026-08-18 07:47 | @backend-dev | Il sito appena pubblicato chiede al database dieci cose che lì non ci sono, e i rimborsi non partono più | 🔴 | Hai unito la richiesta #223 del marketplace alle 05:33 (ora italiana 07:33) e Vercel ha già messo online il codice nuovo: la pubblicazione `AvrsnGUJqyanJkcA3ELJgmxtKeNe` risulta pronta e in produzione. Il codice però è solo metà del lavoro: l'altra metà sono quattro modifiche al database (i file da 114 a 117), che restano da applicare perché toccare il database vero è una firma tua. Verificato dal vivo in sola lettura sul database di produzione: l'ultima modifica applicata è la 113 (`revoke_public_writes_on_read_only_views`), e nessuno dei dieci oggetti che il codice nuovo cerca esiste — la colonna `orders.seller_payout_reversed_cents`, le funzioni `check_coupon`, `release_coupon`, `admin_kpi`, `vuole_notifica`, le viste `sponsored_active_public`, `live_activity_public`, `rider_reviews_ricevute`, la tabella `consent_log`, la colonna `notifications.category`. **Cosa è rotto adesso, in ordine di gravità:** ① **i rimborsi e le contestazioni bancarie**: `refundOrder` chiede quella colonna e riceve un errore, quindi lancia «ordine non trovato» — non funzionano più l'annullamento con rimborso dal pannello, la decisione su un reso, la risoluzione di una contestazione e il rimborso degli ordini scaduti (4 punti nel codice); prima dell'unione funzionavano, quindi è un peggioramento causato dalla pubblicazione; ② **i rimborsi pieni che arrivano da Stripe** non trovano più l'ordine e vengono ignorati in silenzio; ③ **i codici sconto**: chi ne scrive uno valido si sente rispondere «Codice non valido» (l'acquisto in sé funziona ancora — il conto vero lo rifà il server, che legge la tabella per la via diretta); ④ il salvataggio dei consensi sui cookie risponde errore; ⑤ i numeri del tuo pannello di amministrazione e la pagina recensioni del fattorino non caricano. Reggono senza rompersi la vetrina «dal vivo» in home e il carosello degli sponsorizzati: restano semplicemente vuoti. **E soprattutto:** finché le 114 e 115 non sono applicate, i buchi di sicurezza che quella richiesta doveva chiudere sono ancora aperti sul database vero — indirizzi e telefoni dei clienti leggibili senza account, chi si registra come venditore approvato da solo, ordini modificabili dal browser. | supabase | in attesa | I rimborsi ai clienti non partono più da nessuna delle quattro strade del sito: se oggi qualcuno chiede indietro dei soldi, l'operazione fallisce con un errore e i soldi restano fermi. In più il database resta senza le protezioni della richiesta appena unita. | Al tuo via applico i quattro file in ordine (114, 115, 116, 117) al database di produzione e poi rileggo gli stessi dieci oggetti per confermare che ci siano; i file sono già dentro `main`, cartella `migrations/`, e girano su un database vuoto senza errori (118 su 118). Se preferisci farlo tu dall'editor di Supabase, incolla i quattro file in quell'ordine. |
