...
② **Provato oggi, e SI riproduce — questo è il guasto reale:** ho lanciato `node cervello/salute.mjs`, il checkup di salute (sola lettura, niente di rischioso). Mi è stato risposto "richiede approvazione". Ho controllato perché. Il file grande e condiviso (`settings.json`) mi permette di lanciare qualsiasi script `cervello/*.mjs` o `cervello/*.sh`. Ma il tuo file più stretto — `settings.local.json`, quello che vince davvero — elenca per nome SOLO due script: `pulisci-coda.mjs` e `git-pr.mjs`. Ogni altro script chiede un clic di conferma. Tra questi ci sono `giro.sh`, `ritmo.sh`, `ritmo-run.sh`, `monitora.sh`, `salute.mjs` — quelli che il giro, il ritmo e il checkup di salute usano ogni giorno. In un giro automatico, senza nessuno davanti allo schermo, quel clic non arriva mai. Il giro si ferma lì, in silenzio.
