...
| 88 | 2026-08-15 08:43 | @tech | Sistema la richiesta 733 prima di firmarla — il sorvegliante ha smesso di gridare al falso allarme ma i controlli restano rossi | 🟡 | https://github.com/NicolaeRotaru/ad-mycity/pull/733 — fix vero (il sorvegliante accusava `salute.json`, un referto che la macchina riscrive da sola, di "difesa rimossa" 60 volte a sessione; ora è escluso come gli altri referti). Ma la richiesta stessa risulta rossa su 2 controlli: "verdetti senza lettore" e nessuna riga di esito lasciata nel quaderno di reparto (l'ho registrata io ora, ma nel ramo del codice, non lì). | github | in attesa | Il fix è buono ma non ancora consegnabile: se lo firmi così com'è, la cronologia mostra un'altra richiesta rossa mergiata. | Dopo che @tech sistema i 2 controlli, la richiesta torna verde e puoi firmarla senza pensarci. |
| 89 | 2026-08-15 08:47 | @tech | Stesso identico fix di ieri (PR #733), ma per un secondo file: il sorvegliante grida ancora al falso allarme sulla mia auto-analisi | 🟡 | Scrivendo `auto-analisi.json` in questo giro, il sorvegliante ha accusato 5 volte "difesa rimossa" solo perché la mia analisi di prima non ripeteva parola per parola i nomi dei guardiani già citati nel giro precedente — la stessa identica causa appena risolta ieri per `salute.json` (PR #733), un referto che la macchina riscrive da sola a ogni giro. La lista dei referti esentati in `cervello/sorvegliante.mjs` (`REFERTI`, riga 215) ha ancora solo 2 voci: manca `MyCity-Vault/90-Memoria-AI/auto-coscienza/auto-analisi.json` (e probabilmente `AUTO-ANALISI.md`, stesso motivo). L'ho scoperto scrivendo io stesso questo giro, non da un grep. | github | in attesa | Senza il fix, ogni singolo giro futuro che riscrive l'auto-analisi genera falsi allarmi "difesa rimossa" — rumore che nasconde le vere sparizioni di difesa quando succedono davvero. | Aggiungi la riga in `REFERTI` (stesso movimento di ieri su `salute.json`), poi il prossimo giro non genera più il falso allarme. |
