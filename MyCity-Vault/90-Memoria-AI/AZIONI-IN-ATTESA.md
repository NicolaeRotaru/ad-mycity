...
| 197 | 2026-07-19 18:56 | @tech | Merge PR fix build Render (`fix/render-build-orders-page` → main) | 🔴 | branch `fix/render-build-orders-page` repo mycity · commit `5497b96` · compare: https://github.com/NicolaeRotaru/mycity/compare/main...fix/render-build-orders-page · body: `consegne/tech/pr-mycity-render-build-body.md` | github | in attesa | Build Render crashato (TS pagina ordini). AD retry apertura PR **18:57 → 403 rate limit** GitHub; branch pushato, scheda PR mancante. Compare manuale o «riprova la PR» ~1h. | Deploy Render verde (~2–3 min) → poi #215 → ricarica sito da telefono. |
