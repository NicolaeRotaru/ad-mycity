...
| 16 | 2026-07-01 11:05 | operations→supporto | **Eseguire Scelta A ordine zombie €19,05 — Pane Quotidiano** · approvato **`ok 16`** Pannello **2/7 08:38** · **decisione binaria 2/7 17:09 = Scelta A (esegui, non archivia)** · **cena 19–21 SALTATA (agg. 22:20) → riprogrammata MATTINA 3/7** (accorpata al payout-test) | 🔴 | `consegne/operations/2026-07-02-esecuzione-ok16-pranzo.md` | WhatsApp + dashboard + COD (manuale) | ⏳ **IN ATTESA — mattina 3/7** · pranzo+cena 2/7 saltate → confermato esegui · passi #20–#22 | Nicola esegue **domani mattina 3/7** (con il payout-test): partono i contatti reali e la consegna COD — prima transazione MyCity. | Ordine consegnato + COD incassato → A13/A14 recensione + payout-test 03/7. |
| 20 | 2026-07-02 08:38 | operations→supporto | **#16.1 WhatsApp buyer 348 642 1766** — messaggio **slot MATTINA 3/7** (cena 19–21 saltata, agg. 22:20) + chiedi indirizzo reale (placeholder in DB) · link wa.me in pacchetto ok16 | 🔴 | `consegne/operations/2026-07-02-esecuzione-ok16-pranzo.md` § PASSO 1 | WhatsApp (Nicola) | ⏳ **ATTIVO — mattina 3/7** (tap link) | Il buyer riceve il messaggio reale con slot **mattina 3/7** e conferma dove consegnare. | Buyer risponde sì → passi #21–#22. |
...
| 23 | 2026-07-03 02:50 | AD/data-engineer | **Sblocca sensore PostHog (cieco da 11 giri — sentinella 🧠 riverificata il 2026-07-03 08:25)** — nel `.env` c'è `POSTHOG_API_KEY=phc_…`, che è una **Project Key** (solo scrittura eventi): l'endpoint sensore di lettura dà **401 authentication_failed**. Serve una **Personal API Key** (prefisso `phx_`): PostHog → *Settings → Personal API keys → Create* (scope minimo *Project:read*), poi incollala nel VPS come **`POSTHOG_PERSONAL_API_KEY=phx_…`** (il codice la legge già come fallback) e verifica `POSTHOG_HOST` (EU vs US). | 🟡 | `cervello/vps/.env` (riga `POSTHOG_PERSONAL_API_KEY`) · verificatore `cervello/verifica-sensori.mjs:158` | env VPS (incolla + `systemctl restart mycity-worker`) | ⏳ in attesa — la Personal Key la generi tu dal tuo account PostHog | Il sensore funnel/conversione torna a vedere: al prossimo giro il contatore giri-ciechi si azzera e la sentinella 🧠 si spegne. Finché è cieco: nessun numero PostHog scritto come fatto. | Sensore verde → possiamo misurare conversione/eventi del marketplace, non solo i 7 numeri Supabase. |
| 25 | 2026-07-03 11:26 | account-negozi | **Check-in anti-churn Pane Quotidiano** (unico negozio reale) — script retention da agganciare alla chiamata operativa #21 (stessa telefonata 0523 388601, +2 min) + follow-up dopo la consegna. Playbook anti-churn 3/7: **0 negozi con trend −40%**; il churn qui = time-to-first-value, primo ordine fermo **~219h** (peggiorato +50h vs 1/7) → rischio «qui non vendo». **Non duplica #16/#21**: è lo strato-relazione. | 🟡 | `MyCity-Vault/90-Memoria-AI/AZIONI-PRONTE.md` **A6** + **A7** (upsell 🟢 post-consegna) · registro `consegne/account-negozi/2026-07-03-playbook-anti-churn.md` | telefono (rider su #21) | ⏳ in attesa — parte con #16 stamattina | Il titolare di Pane Quotidiano viene rassicurato e tenuto dentro mentre chiudiamo il suo primo ordine: si evita che l'unico negozio reale molli dopo 9 giorni di attesa. | Ordine chiuso + titolare ingaggiato → parte A7 (upsell 2-3 prodotti-civetta) → flusso ordini settimanale. |
