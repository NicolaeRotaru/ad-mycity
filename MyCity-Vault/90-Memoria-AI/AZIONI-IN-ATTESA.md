...
> 🧹 **Housekeeping 2026-07-31 20:21** — Automatico: **66 aperte · 1 chiuse in archivio**.
>
> 🧹 **Housekeeping 2026-07-31 20:21** — Automatico: **66 aperte · 1 chiuse in archivio**.
>
> 🧹 **Housekeeping 2026-07-31 20:21** — Automatico: **66 aperte · 1 chiuse in archivio**.
...
### 🟡 #diagnosi-guardiani-negati — Causa trovata: la riga di permesso non fa quello che sembra fare · ⏳ accodata 2026-07-31 06:29 · diagnosi chiusa 14:35+
**Cosa cambia:** trovata la causa vera (prima era solo un sospetto). La riga in `.claude/settings.json` che dovrebbe dare il via libera a tutti gli script del cervello — `Bash(node cervello/*.mjs:*)` — **non funziona come un jolly**: quell'asterisco in mezzo al percorso non fa match su un nome di file vero, quindi quella riga non approva NESSUNO script per esteso. L'unica ragione per cui `coerenza-fatti.mjs`, `verifica-sensori.mjs`, `allocazione-check.mjs`, `agent-registry-check.mjs` (e pochi altri) funzionano è che sono elencati UNO PER UNO, col nome scritto per intero, poco più sotto nello stesso file. Tutti gli altri — `test-cervello.mjs`, `north-star-check.mjs`, `freschezza-cadenze.mjs`, `apprendimento-guardiano.mjs`, `scadenzario-check.mjs`, `sonda-volano.mjs`, `tasso-lezioni.mjs`, `verifica-automazione.mjs` — restano bloccati non per un freno misterioso ma perché non sono nell'elenco scritto per esteso. Riprovato oggi (14:35+): stesso identico pattern, confermato per la 3ª volta nella giornata.
**Se va bene:** aggiungere alla lista `allow` di `.claude/settings.json` una riga per esteso per ciascuno degli script bloccati (stesso formato di quelle già presenti, es. `"Bash(node cervello/test-cervello.mjs:*)"`), oppure verificare la sintassi glob che Claude Code supporta davvero per i pattern Bash e riscrivere la riga jolly in un formato che funzioni. Con questo fix i cancelli HARD del giro (test, north-star, freschezza cadenze, apprendimento, scadenzario, volano, tasso, automazione) tornano verificabili in automatico invece che a mano — oggi ogni giro perde tempo a "leggere a mano" i JSON che quegli script scriverebbero da soli.
**Nota tecnica:** verificato con prova diretta e ripetuta: `node cervello/coerenza-fatti.mjs` (elencato per esteso) → parte. `node cervello/scadenzario-check.mjs --json` (coperto SOLO dal jolly) → negato «richiede approvazione», anche subito dopo aver appena modificato quello stesso file nella stessa sessione. Nessuna voce di `deny` corrispondente: è un problema di sintassi del pattern `allow`, non una regola che vieta di proposito.
- **Colore:** 🟡 (modifica di un file di configurazione della macchina, non del sito — reversibile, nessun impatto sul marketplace)
- **Reparto:** devops-sre
- **Origine:** `{origine:giro-2026-07-31-0629, osservazione:permessi-incoerenti, diagnosi:giro-2026-07-31-1435, verificato:prova-diretta-ripetuta}`
...
**Contesto:** Nicola ha chiesto (30/7, guardando la scheda Piani) di vedere la data di ultimo aggiornamento su OGNI casella (Business Model Canvas, Piano Editoriale, ecc.), non solo la data globale in cima alla sezione. Il fix è scritto e committato nel branch `fix/data-piani-per-casella` (commit `7e846d78f`): dato che questi file non hanno frontmatter `data:`, la data viene presa dall'ultimo commit git che ha toccato il singolo file (GitHub commits API in produzione, `git log` in locale) — mai un numero inventato. `npx tsc --noEmit` pulito.

**Perché la PR non si apre da questa sessione — CORREZIONE:** al primo tentativo avevo detto "bloccato dai permessi/senza credenziali" — sbagliato, era solo un mio errore di percorso (`node ../cervello/git-pr.mjs` da dentro `pannello/` non combacia col permesso registrato `node cervello/git-pr.mjs`; da radice del repo lo script parte). La causa VERA, trovata dopo: `git-pr.mjs` confronta il branch con `origin/main`, e `origin/main` è fermo da **840 minuti** (~14 ore, vedi il segnale `watch-main: errore` già in coda come [[radiografia-sblocca-pubblicazione]]) mentre `main` in locale ha accumulato 152 commit mai pubblicati. Qualsiasi branch nuovo, anche il più pulito, si porta dietro quei 152 commit di diario automatico nel confronto — ed è esattamente quello che il cancello anti-rumore di `git-pr.mjs` è fatto per bloccare. Non è un problema del mio fix: è la stessa causa radice del blocco di pubblicazione già segnalato.
...
## 🛡️ Supervisione negozi & prodotti — proposte di riempimento (aggiornato 2026-07-31 20:21)
...
> Ultima pulizia: 2026-07-30 12:12 · 1 card totali
...
> Ultima pulizia: 2026-07-30 18:21 · 1 card totali

---

## 🗄️ Archivio — card chiuse

> Ultima pulizia: 2026-07-30 19:35 · 1 card totali

---

## 🗄️ Archivio — card chiuse

> Ultima pulizia: 2026-07-30 20:21 · 1 card totali

---

## 🗄️ Archivio — card chiuse

> Ultima pulizia: 2026-07-30 22:21 · 1 card totali
...
> Ultima pulizia: 2026-07-31 11:51 · 1 card totali

---

## 🗄️ Archivio — card chiuse

> Ultima pulizia: 2026-07-31 12:21 · 1 card totali
...
> Ultima pulizia: 2026-07-31 20:21 · 1 card totali
