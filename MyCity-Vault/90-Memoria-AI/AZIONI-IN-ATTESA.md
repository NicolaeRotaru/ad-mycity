...
| 219 | 2026-07-23 11:26 | @devops-sre | Il token GitHub di ad-mycity non funziona più — nessuna PR di codice può essere aperta | 🟡 | `node cervello/git-pr.mjs` fallisce con "Invalid username or token. Password authentication is not supported" sul repo ad-mycity — il push del branch `fix/ollama-fallback-quota` è bloccato da questo, non da un errore mio. | manuale (VPS) | in attesa | Finché non è risolto, NESSUNA modifica al codice del Pannello/cervello può diventare una PR — tutto il lavoro tech resta fermo in locale. | Rigenera/aggiorna il token GitHub (GIT_PUSH_TOKEN in `cervello/vps/.env`) e verifica con un push di prova; poi riprovo ad aprire la PR Ollama. |
