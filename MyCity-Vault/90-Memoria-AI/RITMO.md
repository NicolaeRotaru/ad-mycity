...
- **Priorità n°2:** **Deploy Sprint 1** in prod. PR #209+#210 mergiate su `mycity/main`. Migrazione `107` + smoke test checkout/fee/RLS/COD. È il prerequisito del batch negozi **6/7**.
- **Priorità n°3:** **Onboarding negozi 6/7.** Nicola inserisce le botteghe. @onboarding-negozi presidia la checklist catalogo/payout/foto.
...
- **Fatto oggi:**
  - ✅ **#19 fix ruoli acquisto MERGED** (PR #211 `f84fc70` → Render LIVE ~08:45). Resta da chiudere lo smoke test di @qa post-deploy.
  - ✅ **Cantiere radiografia macchina** (Cloud Agent 10:15): 18/22 difetti chiusi, PR #138 → main. Voto salute architettura **42→80 ▲**.
  - ✅ **Decisione binaria #16 risolta alle 17:09.** Nicola sceglie **A = ESEGUI** (non «archivia zombie»). Slot spostato a **cena 19–21**.
  - 5 giri AD nel giorno: 08:20 · 10:19 · 16:53 · 17:01 · 17:21.
- **Numeri vs ieri:** invariati. **1** negozio reale (Pane Quotidiano). **0** payout attivi. **5** prodotti. **1** ordine COD €19,05 mai consegnato. **0** pagati, **0** consegnati, **0** payout. **4** buyer (0 nuovi in 7g).
  - ⚠️ **Gap di misura:** live gated (MCP non autorizzato + node non approvato). I 7 numeri sono la **baseline REST delle 10:19, portata avanti**. Nessun numero inventato.
  - **Stallo ~201,5h** (ancora l'ordine del 24/6 08:28). Sono circa 9 ore in più di ieri sera (20:18, a 177,8h).
...
## Punto di mezzogiorno · 2026-09-01 12:00

**Le 3 priorità del mattino:**
1. ❌ **Rimetti online il sito vero** (`#154`/`#155`). È ancora giù, HTTP 503. Dominio e chiavi Vercel aspettano la tua firma.
2. ❌ **Sblocca i pagamenti carta di Pane Quotidiano** (`#182`). Invariato. I tre interruttori Stripe restano spenti.
3. ❌ **Applica le quattro migrazioni al database** (`#184`). Invariato. Non hai ancora scelto quale delle due strade preferisci.

**Correzioni di rotta:** nessuna urgenza nuova. Il quadro è identico da stamattina. È il nono controllo di oggi con lo stesso risultato. L'unico negozio vero resta a zero ordini pagati da 69 giorni.
...
## Piano del mattino · 2026-09-03 08:21

**In una riga:** Stesso piano di 38 minuti fa, richiamato di nuovo: nulla è cambiato, restano le stesse tre firme.
...
**Correzione di rotta:** sui numeri non c'è niente da correggere. Li ho controllati di nuovo ora, sul database vero. Sono identici a stamattina: 1 ordine annullato, 0 pagati, 9 profili, 9 prodotti, 3 carrelli. È il **75° giorno** senza un incasso vero. C'è invece una cosa di processo da correggere. Un controllo automatico chiamato "cadenze" dice "no" da tre passaggi di fila. Nessuno lo sta riparando. Lo strumento che lo ripara è bloccato. È lo stesso buco di permessi delle altre due card gemelle, `#194` e `#195`. L'ho segnalato stamattina alle 10:30 in una scheda nuova, la `#198`. Non ne apro una seconda: è lo stesso buco, non un problema nuovo. Nel pomeriggio non assegno nessun lavoro pesante nuovo ai reparti. C'è un solo negozio vero. Zero pagamenti sono attivi. Spingere ora su marketing o contenuti sarebbe lavoro che nessuno può ancora incassare.
...
## Punto di mezzogiorno · 2026-09-08 12:00

**In una riga:** nono controllo di oggi. Stessi numeri fermi da 77 giorni. L'unica cosa che si muove davvero è la distanza tra la memoria di questo computer e quella su GitHub.

**Le priorità di stamattina, a che punto sono:**
1. ❌ **Sblocca i pagamenti con carta di Pane Quotidiano** (`#182`). Ferma. Resta l'unico blocco confermato tra noi e il primo ordine pagato.
2. ❌ **Riallinea la memoria del VPS con GitHub** (`#199`). Peggiora. Ora 343 scritture di qui non sono mai arrivate là. 12 scritture di là non sono mai arrivate qui. Stamattina alle 6 erano 327. Nasconde già un problema che un'altra sessione aveva risolto altrove.
3. ❌ **Decidi su "Panificio Demo"** (`#196`), il negozio finto comparso nel database. Nessuna risposta nuova.
4. ❌ **Sblocca da fuori questa chat il permesso che tiene fermi i controlli automatici** (`#189`). Emersa durante la mattina. La causa è trovata. Le righe da incollare sono già pronte in coda. Manca solo che qualcuno le metta nel file, da un computer normale, non da qui dentro.

**Correzione di rotta.** Sui numeri non c'è niente da correggere. Li ho ricontrollati ora, in diretta. Sono identici a cinque minuti fa. Sul modo di lavorare sì. Oggi questa macchina ha ripetuto lo stesso controllo pieno nove volte, sempre con lo stesso risultato. È tempo speso a confermare che nulla è cambiato, non a chiudere qualcosa. Da qui in avanti salto la verifica pesante quando i dati sono già confermati uguali da pochi minuti. Nel pomeriggio non assegno lavoro nuovo ai reparti. C'è un solo negozio vero online. Zero pagamenti sono attivi. Spingere ora su marketing o contenuti produrrebbe solo cose che nessuno può ancora incassare.

**Cosa serve da te entro sera:**
- Sblocca i pagamenti con carta di Pane Quotidiano (`#182`). È la mossa numero uno.
- Incolla le righe di permesso della card `#189`. Fallo da un computer o editor normale, non da questa chat. Sblocca in un colpo solo tutti i controlli automatici fermi da una settimana.
- Fai riallineare la memoria del VPS con GitHub (`#199`). Più aspettiamo, più cresce la distanza da recuperare.
