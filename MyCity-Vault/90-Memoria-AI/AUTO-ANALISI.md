# 🔬 AUTO-ANALISI — 2026-08-15 08:45

> Giro completo richiesto in chat. Business invariato, riverificato dal vivo (`verifica-sensori.mjs`,
> 08:40: 1 ordine via REST, stessa firma dal 24/6). Il vincolo HARD `freschezza-cadenze` segnalava che
> il giro delle 06:30 era uscito SALTANDO auto-analisi/apprendimento — questo passaggio lo chiude.

## Voto di fiducia: 87/100 (→ invariato)

**Cosa aveva davvero saltato il giro delle 06:30.** Ho letto i commit veri: `1d40afde3` (06:30) e
`33c60d780` (06:40). Autore: "AD MyCity VPS". Toccano solo file di sensori, sentinelle e Intelligence.
Non è una sessione con il modello dentro. È il pre-step automatico di `giro.sh`, la sonda leggera.
`delta-gate.json` lo conferma: alle 06:23 aveva deciso "nulla di nuovo". Non è stato un errore. Quella
sessione non esisteva come sessione AI. Ma il vincolo `freschezza-cadenze.mjs` non sa fare la differenza
tra sonda leggera e giro pieno. Ha visto `Briefing/2026-08-15.md` fermo al commit delle 00:45 (verificato
con `git log` sul file). Ha segnalato, giustamente, che da quasi 8 ore non usciva un briefing nuovo.
Questo passaggio lo scrive per davvero.

**Trovata una PR aperta 9 minuti prima di questa sessione, mai messa in coda.** Il file
`consegne/tech/pr-ad-mycity-733.md` (creato alle 08:36) non era ancora tracciato da git. Documentava la
PR #733, un fix vero. Il sorvegliante accusava `salute.json` di "difesa rimossa" 60 volte a sessione.
`salute.json` è un referto che la macchina riscrive da sola: non è una difesa spenta. Aggiunta come card
#88 in `AZIONI-IN-ATTESA.md`. `node cervello/ci-stato.mjs` (funzionante in questa sessione) mostra la PR
ancora rossa su 2 controlli. Non l'ho dichiarata pronta per la firma. Ho registrato l'esito nel quaderno
`@tech` con `chiusura-loop.mjs`.

**Scoperto un secondo caso dello stesso identico difetto, riparato ieri su un altro file.** Scrivendo
`auto-analisi.json` in questo passaggio, il sorvegliante ha accusato "difesa rimossa" 5 volte. La causa è
identica a quella della PR #733: un referto che si riscrive da solo, non una difesa spenta. Ma qui il file
è diverso. La lista `REFERTI` in `cervello/sorvegliante.mjs` ha ancora solo 2 voci: `sorvegliante-storico.json`
e `salute.json`. Manca `auto-analisi.json`. Aggiunta come card #89. Stesso movimento di ieri. Non risolvibile
da questa sessione di sola memoria.
...
- Passi 0-1 (sensori/dati) — **FATTA**: `verifica-sensori.mjs` rilanciato dal vivo (08:40), tutti ok tranne `sito_uptime`/`telegram_bot` (invariati, cause note).
- Passo 2 (sentinelle + `verifica-automazione.mjs`) — **NON RITENTATA**: già verificata sufficientemente dai passaggi precedenti di oggi, nessun segnale nuovo da `sensori-cecita.json`.
- Passi 3-4 (radar bidirezionale, delega analista/intelligence) — **NON FATTA APPOSTA**: i 3 file Intelligence sono già freschi (aggiornati 06:40, meno di 2h fa) — riaprirli ora sarebbe ricerca doppia sullo stesso giorno, vietata dal tasso-di-chiusura quando non c'è novità.
- Passo 5-6 (briefing 11 sezioni, STATO, 3 file Intelligence, `ultimo-briefing.json`) — **FATTA**: `Briefing/2026-08-15.md` aggiornato con questo passaggio in cima, STATO.md aggiornato, `ultimo-briefing.json` riscritto. I 3 file Intelligence non toccati (già freschi, vedi sopra).
- Passo 7 (doer mode 🟢/🟡/🔴) — **FATTA**: 🟢 eseguiti (verifica sensori, ci-stato, coerenza-fatti, chiusura-loop); 🟡 nuove accodate (#88, #89).
- Passo 8 (Sala Operativa) — **FATTA**.
- Passo 9 (Piani + `piani-data.mjs`) — **NON FATTA**: nessun piano ha contenuto nuovo da propagare in questo passaggio (nessuna novità di radar/business); script non ritentato per lo stesso motivo.
- Passo 10 (`intenzioni-nicola.json`) — **NON FATTA APPOSTA**: nessuna mossa nuova di Nicola da estrarre rispetto al file esistente.
- Passo 11 (cancello di serietà: `auto-analisi.json` + `AUTO-ANALISI.md`) — **FATTA**: è il cuore di questo passaggio, il gap che il vincolo HARD segnalava.
- Passo 12 (apprendimento) — **FATTA**: 1 lezione nuova (distinzione sonda-leggera vs giro-pieno), esito registrato nel quaderno `@tech` con `chiusura-loop.mjs` (funzionante in questa sessione).
- Passo 13 (auto-miglioramento) — **NON FATTA APPOSTA**: nessun lavoro creativo/pubblicazione in questo giro.
- Passo 14 (sonda auto-radiografia, `sonda-volano.mjs`) — **NON FATTA**: bloccato dall'allowlist, un tentativo.
- Passo 15 (coerenza dei fatti, AR-102) — **FATTA**: `coerenza-fatti.mjs` rilanciato dal vivo, pulito.
- **Vincolo HARD test-cervello** — **NON VERIFICATA**: `test-cervello.mjs` bloccato dall'allowlist in questa sessione, un tentativo; nessun sostituto `node --test` rilanciato in questo passaggio (nessuna modifica di codice propria da verificare).
- **Vincolo HARD freschezza-cadenze** — **PARZIALMENTE FATTA**: chiuso il buco `giro (06:30)`/`ritmo-settimana` scrivendo briefing+auto-analisi+apprendimento veri. `ritmo-mezzogiorno`/`ritmo-sera` restano stale (44h/38h): sono cadenze separate schedulate sul VPS (`cervello/ritmo.md`), non parte del comando "giro" eseguito qui — fuori dal perimetro di questa sessione di sola memoria.
- **Vincolo HARD north-star (AR-113)** — **RISPETTATA**: nessuna azione business discrezionale proposta; il lavoro fatto (auto-analisi, apprendimento, 2 card PR/sorvegliante) sono mandati espliciti di questo turno o correzioni di un difetto trovato sul campo, non ricerca business.
- **Vincolo HARD apprendimento** (promuovi 2-3 lezioni a principio) — **NON FATTA**: `apprendimento-guardiano.mjs` bloccato dall'allowlist.
- **Vincolo HARD correzione-nicola-gate** — **NON RICERCATA in questo passaggio specifico**: `correzione-nicola-gate.mjs`/`gate-veri.mjs` bloccati, un tentativo ciascuno; nessun nuovo candidato gatabile onestamente trovato senza poter verificare.
- **Vincolo HARD freschezza-intelligence** — **VERIFICATA per lettura diretta**: i 3 file Intelligence sono datati 06:40 (meno di 2h), dentro qualunque soglia ragionevole; script non ritentato.
...
**② Diff riletto per intero, non a memoria.** `git status --short` mostra i file toccati da questo
passaggio: `auto-analisi.json`, `AUTO-ANALISI.md`, `AZIONI-IN-ATTESA.md`, `Briefing/2026-08-15.md`,
`STATO.md`, `SALA-OPERATIVA.md`, `ultimo-briefing.json`, `apprendimento.json`, `memoria-squadra/tech.md`,
`memoria-squadra/ad.md`. In parallelo il worker VPS scrive da solo altri file: `sentinella-dati.json`,
`sorvegliante-storico.json`. Hanno timestamp diversi dai miei: non sono lavoro mio.

**③ Prove eseguite, non ricordate.** `node cervello/verifica-sensori.mjs --mcp-supabase=ok` rilanciato
dal vivo alle 08:40. Output letto per intero. `node cervello/coerenza-fatti.mjs` rilanciato dal vivo: verde.
`node cervello/ci-stato.mjs` rilanciato dal vivo alle 08:42: PR #733 rossa, PR #727 verde. `node cervello/
chiusura-loop.mjs registra tech ...` eseguito. Esito confermato dall'output del comando stesso. Tentati dal
vivo e bloccati, un tentativo ciascuno: `north-star-check.mjs --gate`, `correzione-nicola-gate.mjs`,
`gate-veri.mjs`, `tasso-lezioni.mjs`.

**④ L'asticella: un'altra strada era possibile?** Sì. Avrei potuto correggere solo il timbro di
`auto-analisi.json` e `apprendimento.json`, senza contenuto nuovo. Sarebbe stato più veloce. L'ho scartata:
è esattamente il "timbro cosmetico" che l'asticella (AR-128) vieta. La regola chiede una prova che gira,
non una parola scritta. Ho scelto di verificare dal vivo, leggendo i commit reali, la vera natura del giro
delle 06:30. Non ho dato per scontato che fosse un errore. Ho anche controllato `ci-stato.mjs` e
`AZIONI-IN-ATTESA.md` per PR dimenticate, invece di fermarmi al minimo richiesto.

**⑤ Cosa NON ho potuto verificare, dichiarato non inventato.** `test-cervello.mjs` non rilanciato in questo
passaggio, né un suo sostituto `node --test`. Nessuna modifica di codice propria in questo giro da
verificare. Lo script diretto resta bloccato. 7 script HARD restano bloccati dall'allowlist di questa
sessione headless (elenco al punto ①). È lo stesso limite documentato in decine di passaggi precedenti oggi
([[feedback-bash-solo-script-esatti-in-allowlist]]).
