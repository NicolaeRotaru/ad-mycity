# 📅 Eventi & Picchi di domanda — Piacenza

> Aggiornato: **2026-06-30 23:15** · Fonte: @intelligence (giro notturno 30/6, WebSearch + REST live).

---

## 🌦️ Picchi meteo (prossimi 7 giorni)

### 1. TEMPORALI 1/7 — allerta Protezione Civile (confermato)

| Giorno | Max / Min | Condizioni | Leva delivery |
|--------|-----------|------------|---------------|
| **Mar 30/6 (oggi)** | ~35 / 24 | Sereno in serata (~28°C alle 23:08) | fine ondata caldo |
| **Mer 01/7** | ~36 / 20 | **Allerta gialla temporali** + caldo in attenuazione; piovaschi pomeridiani | 🌧️ *"piove, te la portiamo noi"* |
| Gio 02/7 | ~32 / 20 | Variabile, asciutto | clima mite |
| **Ven 03/7 (VP)** | ~33 / 20 | Variabile, **sera mite** | presidio QR comodo |
| Sab 04/7 | ~33 / 21 | Poca pioggia | normale |

Fonti: [Protezione Civile ER 1/7](https://protezionecivile.regione.emilia-romagna.it/novita/notizie/attualita/per-la-giornata-di-1-luglio-allerta-arancione-per-temperature-estreme-in-regione-giallo-temporali) · [UPMeteo Piacenza](https://upmeteo.it/previsioni-meteo-piacenza.htm) · [3BMeteo](https://www.3bmeteo.com/meteo/piacenza).

### 2. VENERDÌ PIACENTINI — 3, 10, 17 luglio

- **Venerdì 3/7 = TRA ~47 ORE** — **49 eventi** quella sera.
- Highlights: ICW Wrestling (P.za Cavalli 22:00), Tu Si Que Avis (P.za Duomo 22:00), Modoolo Square.
- **🟢 Checklist operativa AD:** `consegne/operations/2026-06-30-checklist-countdown-vp-3-luglio.md`
- **Effetto:** presidio offline QR a costo ≈0 (~50-60k persone/sera). Delivery vero il sabato dopo.
  Fonte: [Programma VP PDF](https://www.venerdipiacentini.it/pdf/programma2026.pdf).

### 3. FESTA DEL TORTELLO — Vigolzone, 24-28 luglio

- Fuori raggio consegna; alimenta content DOP/tipici in città.

---

## 🆕 Novità dal 30/6 (23:15)
- **Escalation stallo:** **168h (7 giorni) tra ~11 ore** se nulla cambia — nota AD in `consegne/ad/2026-06-30-nota-escalation-stallo-168h.md`.
- **Countdown VP:** **~46 ore** — urgenza link lista d'attesa invariata.
- **1/7:** allerta temporali ER confermata (invariata vs 23:10).
- **Stallo business:** 156,8h — invariato sul DB (7 numeri = vs 23:10).

## 🚧 ZTL / viabilità
- VP 3/7: centro pedonale/ZTL serale — presidio QR sì, delivery in ZTL no quella sera.

---
> Rigenerato dal giro AD. Prossimo refresh meteo: ogni giro; eventi: settimanale o se cambia calendario.
