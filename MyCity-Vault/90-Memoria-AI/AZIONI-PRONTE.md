...
stato: BOZZE PRONTE — NESSUN INVIO. **Ri-verificato 23/7 11:45 (playbook worker, query dirette DB):** stesso carrello samir invariato (€10, ~875h/~36,5gg fermo) — unico recuperabile. Il carrello demo "Casa Linda" è sparito dalla tabella (pulizia dati, ora 3 record non 4). Profili totali corretti a 7 (il "23" del 20/7 era obsoleto) — 4 buyer confermati. 0 ordini `DELIVERED`, l'unico ordine resta quello CANCELED del 3/7 → gate PQ ancora CHIUSO. Dossier: `consegne/crm/2026-07-23-recupero-carrelli-pronte.md`. Per invio reale: firma Touch #1 🟡 dopo `#ordine-test-pq` + email da `/admin/users`. **Ri-verificato 24/8 13:25 (query dirette DB via MCP).** Ancora gli stessi 3 record in `abandoned_carts`, invariati dal 16/6. Samir (buyer, €10, pesto+2 kefir) resta l'unico reale. Seller-autotest PQ e admin-test restano SKIP, come sempre. Pane Quotidiano ha ancora `stripe_charges_enabled=false` e `stripe_payouts_enabled=false`. Gate ANCORA CHIUSO, nessun invio. Nessuna bozza nuova generata: nulla è cambiato dal 23/7. **Ri-verificato 1/9 11:55 (query dirette DB via MCP, playbook worker):** ancora gli stessi 3 record in `abandoned_carts`, `last_activity` invariata dal 16/6. Samir (buyer, €10 — pesto+2 kefir) resta l'unico reale; seller-autotest PQ e admin-test SKIP come sempre. Pane Quotidiano ha ancora `stripe_charges_enabled=false` e `stripe_payouts_enabled=false` (coerente con card #182 in AZIONI-IN-ATTESA, "PQ non incassa da 18 giorni"). Il congelamento commerciale generale (`registro-fatti.json` → `ripresa.lavoro-operativo`) copre fino a oggi 1/9 incluso. Gate ANCORA CHIUSO su entrambi i fronti (dati + evadibilità). Nessuna bozza nuova generata: nulla è cambiato dal 24/8. **Ri-verificato 2026-09-02 12:06 (query dirette DB via MCP).** Stessi 3 record in `abandoned_carts`, invariati. Samir resta l'unico cliente vero: €10, fermo dal 16/6. Pane Quotidiano ha ancora `stripe_charges_enabled=false` e `stripe_payouts_enabled=false` (vedi card #182, "PQ non incassa da 18 giorni"). Nota: il congelamento generale di Nicola è scaduto il 1/9, ma qui non c'entra — il gate di questa card è solo "PQ evadibile", e resta chiuso da solo. Gate ANCORA CHIUSO. Nessuna bozza nuova: nulla è cambiato dal 1/9. **Ri-verificato 2026-09-03 13:10 (query dirette DB via MCP).** Stessi 3 record in `abandoned_carts`, invariati dal 16/6. Samir resta l'unico cliente vero: €10 (pesto + 2 kefir bio). Pane Quotidiano ha ancora `stripe_charges_enabled=false` e `stripe_payouts_enabled=false` (coerente con card #182, "PQ non incassa da 18 giorni"). Gate ANCORA CHIUSO. Nessuna bozza nuova: nulla è cambiato dal 2/9. **Ri-verificato 2026-09-04 11:45 (query dirette DB via MCP).** Stessi 3 record in `abandoned_carts`, invariati dal 16/6. Samir resta l'unico cliente vero: €10 (pesto + 2 kefir bio). Pane Quotidiano ha ancora `stripe_details_submitted=false`, `stripe_charges_enabled=false`, `stripe_payouts_enabled=false` (coerente con card #182, "PQ non incassa da 18 giorni"). Gate ANCORA CHIUSO. Nessuna bozza nuova: nulla è cambiato dal 3/9. **Ri-verificato 2026-09-05 11:10 (query dirette DB via MCP, playbook worker).** Stessi 3 record in `abandoned_carts`, invariati dal 16/6. Samir resta l'unico cliente vero: €10 (pesto + 2 kefir bio). Pane Quotidiano ha ancora `stripe_details_submitted=false`, `stripe_charges_enabled=false`, `stripe_payouts_enabled=false` (coerente con card #182, "PQ non incassa da 18 giorni"). Gate ANCORA CHIUSO. Nessuna bozza nuova: nulla è cambiato dal 4/9. **Ri-verificato 2026-09-06 12:10** (query dirette sul database, playbook worker). I carrelli abbandonati sono sempre gli stessi 3, invariati dal 16/6. Samir ha ancora il suo carrello vero da 10€: pesto e 2 kefir. Gli altri due non sono clienti: uno è un test del venditore Pane Quotidiano da 13,90€, l'altro è un test dell'account admin da 7,95€. Nessuno dei tre è mai stato recuperato. Pane Quotidiano non può ancora incassare su Stripe: charges e payout restano disattivati (coerente con la card #182, "PQ non incassa da 18 giorni"). Nel database resta un solo ordine, quello zombie del 24/6: annullato, mai consegnato. Il gate resta chiuso. Nessuna bozza nuova: nulla è cambiato dal 5/9. **Ri-verificato 2026-09-07 12:35** (query dirette sul database, playbook worker). Per la prima volta la tabella ha un 4° carrello: €5,00 di pesto genovese, lasciato oggi alle 7:28. Ho controllato chi è prima di preparare qualsiasi bozza — l'email dietro quell'account è nicolaflorea50@gmail.com: sei tu, Nicola, che hai navigato il sito stamattina. Non è un cliente, quindi va scartato come gli altri due test. Samir resta l'unico carrello vero, sempre gli stessi 10€ di pesto e kefir, fermo dal 16/6. Pane Quotidiano non può ancora incassare: charges e payout Stripe restano disattivati (card #182). Il cancello resta chiuso. Nessuna bozza nuova. **Ri-verificato 2026-09-08 12:20** (query dirette sul database, playbook worker). La tabella ha ancora 4 carrelli: samir (€10, invariato dal 16/6) resta l'unico vero. Gli altri tre restano test: il venditore Pane Quotidiano su se stesso (€13,90), l'account admin (€7,95) e il 4° carrello comparso il 7/9 (€5,00 di pesto) — quest'ultimo è ancora tuo, Nicola: stessa email nicolaflorea50@gmail.com, ultimo accesso 7/9 19:34. Pane Quotidiano non può ancora incassare: `stripe_charges_enabled=false`, `stripe_payouts_enabled=false`, `stripe_details_submitted=false`. Il cancello resta chiuso. Nessuna bozza nuova: nulla è cambiato dal 7/9. **Ri-verificato 2026-09-09 11:40** (query dirette sul database via MCP). La tabella ha ancora gli stessi 4 carrelli: samir (€10, pesto + 2 kefir bio, fermo dal 16/6) resta l'unico cliente vero. Gli altri tre restano test, email verificate su `auth.users`: il venditore Pane Quotidiano su se stesso (€13,90, fabrizio.polledri74@gmail.com), l'account admin (€7,95, admin@piacenza-demo.local) e il carrello del 7/9 (€5,00 di pesto) che è ancora tuo, Nicola — nicolaflorea50@gmail.com. Pane Quotidiano non può ancora incassare: `stripe_charges_enabled=false`, `stripe_payouts_enabled=false`, `stripe_details_submitted=false` (coerente con card #182). Il cancello resta chiuso. Nessuna bozza nuova: nulla è cambiato dall'8/9.
...
stato: VERIFICATO 2026-08-14 11:11 (SQL diretta live) — nessun destinatario oggi (10ª verifica di fila con lo stesso esito, dal 14/7): ordini totali **1** (id `58094956…`, CANCELED 3/7, mai consegnato), 0 nuovi ordini dal 24/6, `store_reviews`/`reviews`/`rider_reviews` tutte a **0**. Bozze A13/A14 pronte e invariate per la prima consegna reale. **Nota efficienza (confermata):** questo playbook non ha prodotto nulla di nuovo in 4 verifiche consecutive su un mese — va agganciato come trigger sull'evento DELIVERED (o su `#ordine-test-pq` chiuso), non rilanciato a comando/cadenza finché quel gate non cambia.
...
livello: 🟢 (verifica + bozze) · 🔴 (invio mail Comune/Unione)
canale: email istituzionale + kit WhatsApp negozi
perche: PLAYBOOK Istituzioni worker, 11ª verifica il 7/9 13:10. **Gate ① (bando aperto) ANCORA chiuso**: PI26/BT26/bando Piccole Imprese Territorio tutti chiusi dal 30/7 (PI26 anche non idoneo per MyCity, Nicola 29/7 00:10); bando ER Regione chiuso da giugno (350 domande); nessun bando comunale «rimborso materiali» (mai esistito). 2 WebSearch dirette oggi non hanno trovato nessun bando genuinamente nuovo. **Gate ② (pausa lavoro-operativo Nicola, 24/8-1/9) RISOLTO**: la finestra è passata, confermato dalla card #191 (2/9, "10 azioni-negozio tornate visibili"). Per la regola OR scritta qui sotto dal 20/7 ("il vero sblocco è uno di questi due"), lo sblocco è arrivato — non da un bando nuovo, ma dalla fine della pausa.
preparato: 🏛️ relazioni-istituzionali — `consegne/relazioni-istituzionali/2026-09-07-playbook-bandi-mail-istituzioni.md` (testo riscritto SENZA riferimento a bandi, sostituisce la versione 20/7 che citava PI26 "a sportello") + kit `2026-07-20-kit-bandi-cciaa-negozi-1pagina.md`
bandi_aperti_oggi:
  · NESSUNO — verificato 11 volte (25/6, 6/7, 19/7, 20/7, 21/7, 27/7, 3/8, 10/8, 17/8, 24/8, 7/9 13:10)
...
cosa_cambia: la mail Hub non aveva mai smesso di aspettare — era rimasta orfana dal 30/7 (quando PI26 è diventato inutilizzabile) senza essere mai stata rimessa in coda; il numero che la citava (`#52`) è stato nel frattempo riassegnato a un'altra card. Riscritta oggi come puro "servizio digitale dell'Hub", senza scadenze da citare.
se_va_bene: Nicola mette nome/telefono/email reali al posto dei segnaposto e dà il via; le due mail partono verso Comune e Unione Commercianti.
testo (mail #1 Comune — oggetto aggiornato, nessun bando citato):
MyCity al servizio dell'Hub Urbano — portiamo online le botteghe del centro
testo (mail #2 Unione — oggetto aggiornato, nessun bando citato):
MyCity per i soci dell'Unione — vendita online e consegna, senza costi di attivazione
destinatari:
...
pre-condizioni:
  ...
  · Nessun bando citato (tutti chiusi) — già rispettato nel testo riscritto oggi
coda_canonica: **#202** (2 mail Hub) — pronta, aspetta firma+via Nicola
raccomandazione_ad: gate risolto per metà — bando resta chiuso, ma la pausa che bloccava l'invio è finita da 6 giorni (1/9) e l'azione era rimasta orfana. Non serve un altro giro di ricerca bandi: serve solo la firma di Nicola sui segnaposto.
