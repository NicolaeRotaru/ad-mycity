---
tipo: stato
aggiornato: 2026-07-02 08:17
fonte: AD digitale (7 numeri = live REST 1/7 20:18 · chat Nicola 2/7 07:35+08:16 · Supabase clmpyfvpvfjgeviworth)
---

# 📟 STATO — Cruscotto dell'azienda

> 🧠 **2/7 08:16 — Chat Nicola (casella #19):** «come collego Render?» → **nessun token Render**: deploy marketplace = merge `mycity/main` → auto-deploy GitHub→Render · sblocco AD = **#14+#15 PAT** · via chat **`ok merge fix ruoli-acquisto`** (Approva Pannello su canale Render **non** esegue — mani.ts solo email) · branch #19: **14 file staged, non ancora commit/push**.
>
...
| Deploy marketplace Render | ✅ **Sprint 1 live ~10:31** | GitHub→Render auto · **no API/token Render AD** · Pannello Approva su canale Render **non operativo** (solo Resend) · #19 via chat `ok merge` |
...
- 🟡 Da tenere d'occhio: **#16 ripiano 2/7 mattina**; **#19 deploy ruoli acquisto** (branch staged, commit/push ⏳); **SQL 107 policy**; **#14/#15 token GitHub** (sblocco merge AD, non Render); sync VPS (1× root); MCP cieco (REST mitiga); **1 carrello buyer reale** (samir, 3 interni SKIP); kit bando #12; stallo **oltre 168h (+9,8h)**.
...
1. **Chat 2/7 08:16** — Nicola chiede collegamento Render (#19) → chiarito GitHub auto-deploy + PAT #14/#15 + via chat `ok merge` · L-0702-43.
2. **Chat 2/7 07:35** — Nicola: admin zero acquisti + seller solo pulsante marketplace → branch fix 14 file + spec tech + **#19** accodato 🔴.
3. **Chat 2/7 ~07:09** — Chiariti 4 carrelli operativi: 1 buyer reale (samir), 3 interni/demo SKIP CRM.
4. **Giro 1/7 20:18** — KPI live REST stallo 177,8h (+2,0h). Finestra consegna **CHIUSA**. Escalation v11 🟢 + ripiano 2/7 🟢.
5. **Chat 1/7 20:21** — Nicola «ok 17» → handler worker ✅ · install sudoers bloccato → **1× root** Console Hetzner.
6. **Chat 1/7 20:02** — Nicola «ok configura sync VPS» → codice #17 in repo · spiegati #14/#15.
7. **Giro 1/7 18:18** — Finestra consegna APERTA → #16 entro 20:00 (non eseguito).
8. **Pannello 1/7 11:05** — Nicola **Scelta A** ordine zombie → **#16** in attesa
...
2. [ ] 🔴 **#19 — Deploy fix ruoli acquisto** — admin zero acquisti + seller solo «Vai al marketplace» (**`ok merge fix ruoli-acquisto`** · branch staged, commit/push ⏳ · Render auto post-merge)
