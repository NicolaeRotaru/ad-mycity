...
- [2026-07-11] Il remote di `ad-mycity/marketplace` non ha credenziali: prima del push set-url con token → `TOKEN=$(grep GIT_PUSH_TOKEN /opt/mycity/ad-mycity/cervello/vps/.env | cut -d= -f2) && git remote set-url origin "https://NicolaeRotaru:${TOKEN}@github.com/NicolaeRotaru/mycity.git" && git push --force-with-lease origin fix/NOME-BRANCH`. Il .env di ad-mycity sta in `cervello/vps/.env` (NON in `vps/.env` — non esiste).
- [2026-07-11] Dopo aver pushato un branch vecchio (es. Sprint 1 del 1/7, pushato solo al 11/7), la PR avrà quasi certamente conflitti di merge perché `main` è andato avanti. Verificare SEMPRE i conflitti subito dopo l'apertura della PR (via `gh api` o WebFetch GitHub API) e proporre il rebase a Nicola come passo immediatamente successivo, non nella prossima chat.
- [2026-07-11] Committare in `ad-mycity/marketplace` come `root` richiede DUE fix in sequenza: ① `git config --global --add safe.directory /opt/mycity/ad-mycity/marketplace` (dir owned da utente diverso) → ② `git config --global user.email "nicolaflorea50@gmail.com" && git config --global user.name "AD MyCity"` (root non ha identità git). Entrambi prima del `git add -A && git commit`.
