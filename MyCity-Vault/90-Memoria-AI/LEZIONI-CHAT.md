...
- [2026-07-11] Il remote di `ad-mycity/marketplace` non ha credenziali: prima del push set-url con token → `TOKEN=$(grep GIT_PUSH_TOKEN /opt/mycity/ad-mycity/vps/.env | cut -d= -f2) && git remote set-url origin "https://NicolaeRotaru:${TOKEN}@github.com/NicolaeRotaru/mycity.git" && git push --force-with-lease origin fix/NOME-BRANCH`. Da fare ogni volta che si lavora su quel checkout.
