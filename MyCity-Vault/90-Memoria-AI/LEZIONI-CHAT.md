...
- [2026-07-11] Le chat del Pannello non vengono salvate finché Vercel non ha `SUPABASE_URL` + `SUPABASE_SERVICE_KEY` del progetto Supabase Memoria come variabili d'ambiente — il codice `/api/conversazioni` + `store.ts` esiste già; senza quelle chiavi ogni ricarica cancella la cronologia. Nicola usa il Pannello online (non VPS).
