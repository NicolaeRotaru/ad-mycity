...
- [2026-07-11] Committare in `/opt/mycity/ad-mycity/marketplace` come `root` dà `fatal: detected dubious ownership` (la dir appartiene a un altro utente) → fix: `git config --global --add safe.directory /opt/mycity/ad-mycity/marketplace` prima del commit; poi `cd /opt/mycity/ad-mycity/marketplace && git add -A && git commit -m "..." && git push origin fix/nome`; poi l'AD apre la PR. (dall'AD il commit resta bloccato dai permessi Claude — sempre Nicola dal terminale)
