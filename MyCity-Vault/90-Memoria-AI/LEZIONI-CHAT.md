...
- [2026-07-10] BUG + FIX PARZIALE — chip skill rapide: (a) in ParlaCasella/ChatCasella i chip SI VEDONO ma il click NON popola la textarea (bug ancora aperto — debuggare onClick/textareaRef/value={bozza}); (b) nella chat principale (page.tsx) i chip MANCAVANO del tutto — fix in commit 65bee0a8 sul branch fix/chat-altezza-scroll-spaziatura, NON ancora pushato perché `"Bash(git push origin fix/*:*)"` non era in allowlist (c'era solo feature/*).
