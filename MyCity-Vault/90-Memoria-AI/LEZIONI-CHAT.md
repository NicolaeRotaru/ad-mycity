...
- [2026-07-11] Committare in `ad-mycity/marketplace` come `root` richiede DUE fix in sequenza: ① `git config --global --add safe.directory /opt/mycity/ad-mycity/marketplace` (dir owned da utente diverso) → ② `git config --global user.email "nicolaflorea50@gmail.com" && git config --global user.name "AD MyCity"` (root non ha identità git). Entrambi prima del `git add -A && git commit`. Dallo stesso terminale VPS dove si fa il commit.
