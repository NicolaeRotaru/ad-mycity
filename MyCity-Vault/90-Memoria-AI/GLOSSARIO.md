...
Parole **del futuro, non di oggi**: oggi la macchina è una sola. Stanno qui perché il nome «worker»
era già occupato — è il processo sul VPS che svuota la coda — e chiamare *worker* anche il servizio
ai negozianti avrebbe confuso due cose diverse per sempre.

| Parola | Significato |
|---|---|
| **CENTRO** | La macchina che dirige **l'azienda**: strategia, soldi, i 120 senior, la memoria unica, la Cabina di Nicola. È questo repo, alleggerito. Non tocca il sito, non parla coi clienti. |
| **PIAZZA** | La macchina che manda avanti **il marketplace** — catalogo, prezzi, ordini, bug, salute. È l'unica che ha le **chiavi in scrittura** sul sito. **Una per città**: Parma sarà una seconda PIAZZA, stesso codice. |
| **BOTTEGA** | La macchina venduta a canone **ai negozianti**: risponde ai loro clienti, tiene il loro catalogo, scrive i loro post. **Una sola per tutti i negozi.** |
| **Multi-negozio** | Il modo in cui una macchina serve N clienti senza duplicarsi: *quello che è uguale per tutti è codice, quello che è diverso è un dato con sopra il nome del negozio*. Il negoziante numero 41 non fa nascere una macchina: fa nascere **una riga**. |
| **`negozio_id`** | Il nome del negozio su **ogni riga di ogni tabella**. Senza, il muro dei dati non ha su cosa poggiare. |
| **Muro dei dati (RLS)** | Il database **rifiuta** di dare a un negozio le righe di un altro. Non è il buon senso dell'AI a separarli: è il database. Il marketplace lo usa già perché un venditore veda solo i suoi ordini. |
| **Corsia del negozio** | Una coda sola, servita **a turno** tra i negozi: chi accoda 200 lavori non lascia gli altri 40 ad aspettare. Ogni negozio ha quota, tetto di spesa e interruttore propri. |
| **Mandato / referto** | Come si parlano due macchine. **Mandato** scende (obiettivi, budget, cosa puoi 🟢🟡🔴), **referto** sale (numeri con la fonte, cosa ho fatto, cosa chiedo). Il CENTRO legge il referto, **non** il marketplace. |
| **Firma del negoziante** | I 🔴 di un negozio li firma **il negoziante**, non Nicola. Senza questa regola Nicola firmerebbe 400 card al giorno e il sistema morirebbe lì. |

📄 Il disegno intero, col piano e le sette prove di collaudo: [[ARCHITETTURA-TRE-MACCHINE]].
