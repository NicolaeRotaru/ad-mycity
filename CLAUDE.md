...
- **"contenuti pro: [quali]"** (alias: *"fammi i contenuti pro"*, *"modalità esperto"*) → produci i contenuti richiesti a **qualità da social manager 10+ anni** (≥ serie W1–W5, punta più in alto). Per OGNI contenuto: ① scegli il canale per ROI reale (a 0 follower → gruppi FB locali, non IG); **hook che ferma lo scroll**, copy nativo, **1 sola idea per grafica**, CTA, primo commento col link su FB, tattica di pubblicazione + KPI onesti. ② **Modella sui format vincenti dei competitor** (`consegne/intelligence/5-aziende-simili-social.md`). ③ **Renderizza la grafica VERA** con la Content Factory (`cervello/content-factory/`: `render.mjs` + i template per categoria — manifesto-causa, storia-bottega, carosello-edu, prodotto-dop, pov-relatable, prova-sociale-quote, contatore-civico…); per i reel consegna **script + cover**. ④ Rispetta il **brand DNA** (colori, font Fraunces/Inter, tagline, @handle) e le **regole onestà**: niente numeri/testimonianze inventati, segnaposto per i dati di Garetti, esempi marcati. ⑤ Delega a **coppie di senior** (dominio + designer/ai-designer) con auto-revisione qualità; poi **QA visivo finale tuo (AD)** e rigenera se non è all'altezza. ⑥ Salva in `consegne/content/` + `creativi/output/social/`, **accoda le pubblicazioni 🔴** in [[AZIONI-IN-ATTESA]], aggiorna memoria, committa, e consegna **grafiche + sintesi + "cosa serve da Nicola"**. Colore: 🟢 creare/renderizzare · 🔴 pubblicare.
