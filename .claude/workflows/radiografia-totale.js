...
const GIA = '/tmp/claude-0/-home-user-ad-mycity/e06d800a-8299-5f3f-93ce-af92fd359bc3/scratchpad/gia-trovati.md'
...
// ─────────────────────────────────────────────────────────────────────────────
const MANDATI = [
  { organo: 'macchina', key: 'agenti-routing', focus: 'I 120 mansionari in .claude/agents/: buchi di copertura, doppioni di mandato senza deferral, description che si contendono la stessa domanda, agenti mai richiamati da CLAUDE.md/COMANDI.md, deferral che puntano a un collega inesistente.' },
  { organo: 'macchina', key: 'memoria-contratti', focus: 'Il vault (90-Memoria-AI, memoria-squadra): contraddizioni fra file, JSON divergenti dai contratti di cervello/auto-coscienza.md, campi obbligatori vuoti (es. severita non popolata nel cantiere), id duplicati, file morti che nessuno legge più.' },
  { organo: 'macchina', key: 'sensori-cecita', focus: 'I sensori: chi dichiara "ok" senza aver misurato, riassunti che contraddicono il dettaglio, cecità travestita da verde, sensori spenti contati come rotti (e viceversa), un comando di sola lettura che riscrive lo stato peggiorandolo.' },
  { organo: 'macchina', key: 'volano-cadenze', focus: 'Il volano dell apprendimento e le cadenze: lezioni senza un freno che possa fallire, correzioni di Nicola chiuse con una frase invece che con un gate, cadenze dichiarate in ritmo.md che non lasciano tracce reali, quaderni fermi.' },
  { organo: 'macchina', key: 'semaforo-northstar', focus: 'Il semaforo 🟢🟡🔴 e l allineamento alla crescita: punti dove un 🔴 può partire senza firma, colore deciso a occhio invece che da una regola, quota di lavoro che la macchina fa su sé stessa invece che su ordini/negozi/margine.' },
  { organo: 'macchina', key: 'rischio-se-stessa', focus: 'La macchina può farsi male: segreti esposti, permessi .claude/settings.json troppo larghi, un giro che può corrompere la memoria (reset/force-push), loop auto-amplificanti, single point of failure, guardiani che passano quando il dato manca.' },

  { organo: 'pannello', key: 'stato-freschezza', focus: 'pannello/: stato perso cambiando chat o ricaricando, liste stale, dati non aggiornati dopo una mutazione, caselle con dato condiviso senza usePanelSync/emitSync, race condition, doppio invio, useEffect senza cleanup.' },
  { organo: 'pannello', key: 'errori-navigazione', focus: 'pannello/: navigazione e tasto indietro, deep-link rotti, errori non gestiti, promise senza catch, crash su dato mancante, error boundary assenti, messaggi di errore che non dicono cosa fare.' },
  { organo: 'pannello', key: 'card-mobile-perf', focus: 'pannello/: le card di approvazione (mostrano "Cosa cambia"/"Se va bene"? titoli con sigle o path che Nicola non deve vedere?), mobile e accessibilità, render inutili, liste non virtualizzate, dipendenze vulnerabili.' },

  { organo: 'senior', key: 'registro-qualita', focus: '.claude/agents/: il conteggio torna con agent-registry-check.mjs? frontmatter validi? i mansionari dicono COME si lavora o solo COSA si sa? mancano output atteso, colore, dove scrivere? quali senior non sono MAI stati richiamati (incrocia con memoria-squadra/, consegne/, DECISIONI.md)?' },
  { organo: 'senior', key: 'loop-collaborazione', focus: 'I quaderni memoria-squadra/: righe ESITO presenti? quanti reparti fermi o decorativi? la calibrazione atteso→reale esiste? i senior si passano il lavoro tra loro o finisce tutto da Nicola? cerca i passaggi mancanti nella Sala Operativa.' },

  { organo: 'worker', key: 'coda-lock-orfani', focus: 'cervello/: il claim dei lavori è atomico o due worker possono prendere lo stesso? i lavori orfani tornano indietro da soli? il lock è davvero esclusivo dopo un crash (file stale, PID riusati, nessuna scadenza)? il kill-switch della pausa è fail-closed?' },
  { organo: 'worker', key: 'git-motore-timer', focus: 'cervello/: dove si perde il lavoro (rebase/force-push, rami divergenti, gate di pubblicazione che blocca per conflitti inesistenti); cervello/motore-ai.sh quando la quota finisce, le credenziali scadono o scatta il timeout (un errore del motore diventa un "fatto" silenzioso? retry senza backoff?); i timer e cervello/ritmo.sh fanno quello che dicono?' },
  { organo: 'worker', key: 'script-shell-risorse', focus: 'cervello/*.sh (10 file): set -euo pipefail mancante, variabili non quotate, exit code ignorati, errori in /dev/null che diventano verdetti, race su file condivisi; più log e cartelle che crescono senza rotazione né potatura.' },

  { organo: 'github', key: 'ci-segreti-igiene', focus: '.github/workflows/ (4 file): trigger che non scattano quando dovrebbero, path filter troppo stretti, job senza timeout, permissions larghe, azioni non pinnate a un SHA; più segreti committati, .gitignore incompleto, artefatti generati tracciati che generano conflitti a ogni ramo.' },
  { organo: 'github', key: 'hook-locali', focus: 'Gli hook locali: .claude/settings.json e cervello/installa-hooks.sh — un hook che fallisce blocca il lavoro o passa in silenzio? gli hook installati coincidono con quelli dichiarati? un hook scrive mentre il comando è di sola lettura? un hook può essere saltato senza che nessuno se ne accorga?' },

  { organo: 'codice', key: 'guardiani-scritture', focus: 'cervello/*.mjs (185 file): guardiani che danno un verdetto senza aver misurato; scritture in memoria non atomiche (due processi corrompono il JSON, nessun backup prima di sovrascrivere); catch vuoti, exit 0 su fallimento, JSON.parse senza try; logica duplicata che divergerà.' },
]
...
- NON ripetere difetti già noti: leggi ${GIA} (31 già trovati) e
  MyCity-Vault/90-Memoria-AI/auto-coscienza/cantiere-difetti.json (563 voci). Cerca il NUOVO.
${BUDGET}`
...
function promptFinder(d, giro, giaTrovati) {
  ...
  return `Sei un revisore senior spietato. Organo "${d.organo}", mandato "${d.key}".
...
function promptVerifica(f) {
  return `Sei un verificatore avversariale. REFUTA questo difetto, non confermarlo. Parti dal
presupposto che sia un falso allarme.
...
const critici = await parallel(ORGANI.map(o => () => {
  ...
  return agent(`Critico di completezza per l'organo "${o}" (repo ${REPO}).
...
${REGOLE}`, { label: `critico:${o}`, phase: 'Critico di completezza', schema: SCHEMA_DIFETTI })
}))
