...
const DIMS = [
  ...
  { key: 'stato-persistenza', senior: 'frontend-dev', focus: 'perdita di stato: le RISPOSTE del worker/chat SPARISCONO cambiando conversazione o tab; stato tenuto in useState locale che si azzera al re-render/smontaggio invece che in memoria server (conversazioni) o storage; race tra fetch e cambio-chat che scrive la risposta nel contenitore sbagliato; messaggi non ricaricati quando torni su una chat.' },
  { key: 'freschezza-dati', senior: 'frontend-dev', focus: 'dati stale: liste (lavori, azioni "cose da fare", proposte) che NON si aggiornano in tempo reale; manca polling/refetch o revalidazione dopo un\'azione (approva/riprova/ignora); uso di cache invece di cache:no-store; elementi già fatti/spuntati che restano nella lista; nessun refresh dopo mutazione.' },
  { key: 'stati-async', senior: 'frontend-dev', focus: 'stati di caricamento/vuoto/errore: fetch senza gestione errore, spinner infiniti, nessun feedback dopo un clic, bottoni non disabilitati durante l\'invio (doppio clic → doppia azione), optimistic update che non fa rollback in caso di errore.' },
  { key: 'coerenza-azioni', senior: 'backend-dev', focus: 'i flussi azione (api/approva, /riprova, /proposta, /decisione-ordine): idempotenza (doppia approvazione = due lavori?), la card riflette lo stato reale dopo l\'azione, dedup delle proposte già decise, gestione del caso "memoria non collegata".' },
  { key: 'robustezza-errori', senior: 'qa', focus: 'robustezza runtime: accessi a proprietà possibilmente null/undefined, JSON.parse senza try, date invalide (new Date su valore mancante → NaN), array .map su possibile undefined, chiavi React mancanti/duplicate, useEffect con dipendenze sbagliate che rifà fetch all\'infinito o mai.' },
  { key: 'accessibilita-mobile', senior: 'accessibility', focus: 'mobile/PWA/a11y: target touch troppo piccoli, gesture "indietro" su mobile, focus/tastiera, contrasto, testo che va fuori, safe-area, elementi non raggiungibili; il Pannello è usato da telefono.' },
  { key: 'performance-render', senior: 'cro', focus: 'performance percepita: re-render inutili, useMemo/useCallback con dipendenze sbagliate (es. dep [lavori] che legge window), liste lunghe senza virtualizzazione, refetch a cascata, componenti pesanti montati sempre.' },
]
