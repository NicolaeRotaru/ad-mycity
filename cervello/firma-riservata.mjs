...
const RE_METODO_SCRITTURA = /method:\s*"(?:POST|PATCH|PUT)"/i;
...
export function scriveImpostazioni(testo = "") {
  ...
  return righe.some((r, i) => {
    ...
    // il metodo può stare sulla riga dopo: si guarda una finestra corta
    const finestra = righe.slice(i, i + 6).join("\n");
    return RE_METODO_SCRITTURA.test(finestra);
  });
}
...
export function difettiFirma(testo = "", nome = "?") {
  ...
  if (scriveImpostazioni(t)) {
    ...
    else for (const c of chiavi) if (riservataANicola(c)) d.push({ file: nome, difetto: "firma_scritta_dal_cervello", chiave: c });
  }
