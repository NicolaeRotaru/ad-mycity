...
- Modifica al CODICE (Pannello o cervello)? MAI committare o pushare su main. Strada UNICA: parti da main (git checkout main — è GIÀ allineato: watch-main lo aggiorna ogni 5 minuti; NIENTE git pull o fetch, il remote è volutamente senza credenziali), git checkout -b fix/nome-parlante → committa lì → scrivi la descrizione PR (cosa/perché/come provare) in consegne/tech/pr-ad-mycity-body.md o passa --body → node cervello/git-pr.mjs --repo ad-mycity --base main --accoda → dai a Nicola il link della PR (il merge lo firma lui dal Pannello). Dopo la PR torna su main (git checkout main): la sessione successiva non deve ereditare il tuo branch.
...
Se il lavoro tocca il CODICE: branch dedicato + descrizione PR obbligatoria (file in consegne/tech/ o --body) + node cervello/git-pr.mjs --repo ad-mycity --accoda (mai commit o push su main; il merge lo firma Nicola dal Pannello).
