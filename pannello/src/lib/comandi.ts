...
export const PROMPT_GIRO =
  ...
  "⚠️ Pubblica la memoria SOLO sul ramo memoria-ad (PR con base memoria-ad, MAI main). " +
...
export function messaggioLavoroInCorso(tipo: string): string {
  if (tipo === "giro") {
    return (
      ...
      "La memoria verrà pubblicata sul ramo **`memoria-ad`** (non su main). " +
      "Segui l'avanzamento in **«Lavori del cervello»** qui sotto; quando finisce, la risposta compare anche qui."
    );
  }
