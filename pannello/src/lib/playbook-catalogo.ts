// 🗡️ IL CATALOGO dei playbook dell'arsenale: dato puro, senza nessun import.
//
// Perché sta qui e non dentro `playbook.ts`: il catalogo è anche l'ANAGRAFE DEI NOMI. Quando un
// lavoro di tipo `playbook` arriva nella lista del Pannello, l'unico modo per chiamarlo col suo
// nome vero («Recupero carrelli abbandonati») invece che col nome della sua specie («playbook») è
// riconoscere il compito qui dentro. `playbook.ts` però parla col database (store.ts): tirarselo
// dietro fin dentro il browser porterebbe codice di server in una pagina, e un giro di import
// circolari. Il dato sta da solo, e chi gli serve se lo prende — server o browser che sia.
